...
const Navbar = () => {
    ...
    const {setShowSearch, showSearch, getShoppingBagCount} = useContext(ShopContext);
    ...
    <div className='relative flex items-center justify-between py-5 font-medium'>
        ...
        <div className='flex items-center gap-6'>
            ...
            <div className='group relative'>
                <Link to={'/login'}>
                    <img src={assets.profile_icon} className='w-5 cursor-pointer' alt='Cart' />
                </Link>
        ...
        <div className={`absolute top-0 right-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
            <div className='flex flex-col text-gray-600'>
                <div className='flex items-center gap-4 p-3'>
                    <img className='h-4' src={assets.dropdown_icon} alt='Close' onClick={()=> setVisible(false)} />
                    <p>Close</p>
                </div>
                ...
                <NavLink onClick={()=> {setVisible(false)}} className='py-2 pl-6 border-b' to='/contact'>CONTACT US</NavLink>
            </div>
        </div>
    </div>
  )
